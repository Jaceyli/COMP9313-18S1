    ...
    public static class StripeMapper extends Mapper<Object, Text, Text, MapWritable>{
        ...
        public void map(Object key, Text value, Context context) throws IOException, InterruptedException{
            ...
            for(int i = 0; i < arrayList.size(); i++){
                ...
                // For example, give a line (b, c, d, a), for term b, we create a MapWritable object bMap.
                // Next we process each term appearing after b in the same line.
                // For (b, c), we put (c, 1) to bMap.
                // For (b, d), we put (d, 1) to bMap.
                // For (b, a), b > a (alphabetical order),thus, we create a MapWritable object aMap,
                // and put (b, 1) to aMap, and emit (a, aMap) to the reducer.

                    if(word1.compareTo(word2) < 0){
//                        如果word1比较小，word2加入word1的 mapwritable record中。
...
//                        如果word2比较小，新建一个word2的mapwritable record，写入{word1 : 1}
    ...
    public static class StripeCombiner extends Reducer<Text, MapWritable, Text, MapWritable> {
        public void reduce(Text key, Iterable<MapWritable> values, Context context) throws IOException, InterruptedException{
            ...
            for(MapWritable val : values){
                Set<Map.Entry<Writable, Writable>> sets = val.entrySet();
                for(Map.Entry<Writable, Writable> entry: sets){
...
//                System.out.println(word.toString()+" "+count.toString());
    ...
    public static void main(String[] args) throws Exception {
        ...
        Job job = Job.getInstance(conf, "CoTermSynStripe");
